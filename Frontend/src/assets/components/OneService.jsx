...
const OneService = () => {
  ...
  useEffect(() => {
    const fetchService = async () => {
      try {
        ...
        setService({ ...response.data, rating: 4 }); // Set default rating for display
      } catch (err) {
  ...
  const submitRating = async () => {
    try {
      await axios.post(`http://127.0.0.1:8000/api/services/${id}/rate/`, {
        rating: userRating,
      });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <RatingIcon rating={userRating} onRate={handleRatingChange} />
